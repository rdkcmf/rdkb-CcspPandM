...
#if defined _COSA_DRG_TPG_ || _COSA_BCM_MIPS_
    ...
    )
{
    ...
    if (bEvent)
    {
        v_secure_system("sysevent set multinet-syncMembers %d", pBridge->l2InstanceNumber);
    }
    return ANSC_STATUS_SUCCESS;
#endif
}
...
ANSC_STATUS SWBrRemovePort(struct bridge* bridge, ULONG portInstanceNumber) {
    ...
    v_secure_system("sysevent set multinet-syncMembers %d", bridge->l2InstanceNumber);
    return ANSC_STATUS_SUCCESS;
}
