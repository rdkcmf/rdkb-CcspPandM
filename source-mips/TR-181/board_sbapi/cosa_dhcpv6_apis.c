...
static int _dibbler_client_operation(char * arg)
{
    if (!strncmp(arg, "stop", 4))
    {
      ...
      #endif
#else
        v_secure_system("killall %s", CLIENT_BIN);    
        sleep(2);   
        v_secure_system("killall -9 %s", CLIENT_BIN);
#endif
    }
...
char * CosaDmlDhcpv6sGetStringFromHex(char * hexString){
    ...
    while( hexString[i] && (i< sizeof(newString)-1) )
    {
        buff[k++]        = hexString[i++];
        if ( k%2 == 0 ) {
             newString[j++] = (unsigned char )strtol(buff, (char **)NULL, 16);
    ...
    )
{
    ...
    system("sh /etc/utopia/service.d/service_routed.sh zebra-restart");
    #endif
...
static void *InterfaceEventHandler_thrd(void *data)
{
    ...
    /*CID: 121121 Missing return statement*/
    return NULL;
				...
				while((token = strtok_r(pt, ",", &pt)))
				 {
					...
					if(GenIPv6Prefix(token,v6Tpref,out1))
					{
						...
						if(tbuff[strlen(tbuff)-1] == '0')
						{
							...
							v_secure_system("ifconfig %s down;ifconfig %s up",token,token);
						}
