...
//Prash
    if( AnscEqualString(ParamName, "MeshRedirectorURL", TRUE))
    {        
        /* collect value */
    ...
    )
{
    ...
    /* check the parameter name and set the corresponding value */
    if (strcmp(ParamName, "ReportingInterval") == 0)
    {
        char buf[16]={0};
        errno_t rc = -1;
 ...
 if( AnscEqualString(ParamName, "Enable", TRUE))
    {
        ...
        errno_t rc       = -1;
        int     ind      = -1;
        if( syscfg_get(NULL, "UPnPxPKI", value, sizeof(value)) == 0 )
        {
            if( value != NULL )
            {
                 rc = strcmp_s(value,sizeof(value),"true",&ind);
                 ERR_CHK(rc);
                 if((!ind) && (rc == EOK))
                 {
                     *pBool = TRUE;
        ...
        {
            CcspTraceError(("syscfg_get failed for UPnP xPKI\n"));
        }
    }
  return FALSE;
}
